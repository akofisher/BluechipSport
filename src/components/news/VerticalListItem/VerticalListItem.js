...
import { View, TouchableOpacity, StyleSheet, Share, Dimensions } from "react-native";
...
const VerticalListItem = React.memo((props) => {
  ...
  return (
    <TouchableOpacity style={st.item} onPress={onPress} key={id + listKey + index}>
      ...
      <View style={st.itemTextContent}>
        ...
        {isTeamNews || isPopularLeague ? (
          ...
          <View style={[cxs.row, cxs.alignCenter]}>
            {teamImg ? (
              <FastImage style={[cxs.w17, cxs.h17]} source={{ uri: teamImg || null }} />
            ) : null}
            <Text style={[teamImg ? cxs.ml10 : "", st.itemDate]}>{formattedDate}</Text>
          </View>
        )}
      </View>
    </TouchableOpacity>

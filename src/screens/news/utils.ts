import { Articles } from '../../store/slices'

export const processGetArticlesResponse = (data: any): Articles => {
  ...
  data.forEach((element) => {
    articles.push({
      ...
      isSquareImage:
        element.main_gallery_item?.image_x ===
        element.main_gallery_item?.image_y,
      ...
      transferPlayerName: element.players?.length
        ? element.players[0]?.common_name
        : '',
      transferPlayerImage: element.players?.length
        ? element.players[0]?.image_path
        : '',
